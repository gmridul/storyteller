...
def _extract_argmax_and_embed(embedding, aux,
  ...
  def loop_function(prev, _):
    ...
    # embedding_lookup.
    emb_prev = tf.concat([embedding_ops.embedding_lookup(embedding, prev_symbol), tf.expand_dims(aux[0], axis=1)],axis=1)
...
def embedding_rnn_decoder_aux(decoder_inputs,
                          aux,
                          initial_state,
                          cell,
                          num_symbols,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_decoder") as scope:
    ...
    loop_function = _extract_argmax_and_embed(
        embedding, aux, output_projection,
        update_embedding_for_previous) if feed_previous else None
    
    emb_inp = (tf.concat([embedding_ops.embedding_lookup(embedding, decoder_inputs[i]), tf.expand_dims(aux[i], axis=1)], axis=1)
               for i in range(len(decoder_inputs)))
    #print(tf.concat([embedding_ops.embedding_lookup(embedding, decoder_inputs[0]), tf.expand_dims(aux[0], axis=1)], axis=1).get_shape())
    return rnn_decoder(
        emb_inp, initial_state, cell, loop_function=loop_function)

def embedding_rnn_seq2seq_aux(encoder_inputs,
                          decoder_inputs,
                          aux,
                          cell,
                          num_encoder_symbols,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
    ...
    if isinstance(feed_previous, bool):
      return embedding_rnn_decoder_aux(
          decoder_inputs,
          aux,
          encoder_state,
          cell,
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        outputs, state = embedding_rnn_decoder_aux(
            decoder_inputs,
            aux,
            encoder_state,
            cell,
