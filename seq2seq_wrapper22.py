...
class Seq2Seq(object):
    ...
    def __init__(self, xseq_len, yseq_len, pseq_len,
            xvocab_size, yvocab_size,
            emb_dim, num_layers, ckpt_path,
            lr=0.0001, 
            epochs=1000, model_name='seq2seq_model_22'):
        ...
        def __graph__():
            ...
            self.labels_fwd = [ tf.placeholder(shape=[None,], 
                            dtype=tf.int64, 
                            name='eif_{}'.format(t)) for t in range(yseq_len) ]
            ...
            # Basic GRU cell wrapped in Dropout Wrapper
            ...
            with tf.variable_scope('decoder_fwd') as scope:
                ...
                self.decode_outputs_fwd, self.decode_states_fwd = tf.contrib.legacy_seq2seq.embedding_rnn_seq2seq(self.enc_ip,self.dec_ip_fwd, stacked_gru,
                ...
                self.decode_outputs_test_fwd, self.decode_states_test_fwd = tf.contrib.legacy_seq2seq.embedding_rnn_seq2seq(
                    self.enc_ip, self.dec_ip_fwd, stacked_gru, xvocab_size, yvocab_size,emb_dim,
                    feed_previous=True)

            with tf.variable_scope('decode_bwd') as scope:
            	# build the seq2seq model 
                #  inputs : encoder, decoder inputs, LSTM cell type, vocabulary sizes, embedding dimensions
                self.decode_outputs_bwd, self.decode_states_bwd = tf.contrib.legacy_seq2seq.embedding_rnn_seq2seq(self.enc_ip,self.dec_ip_bwd, stacked_gru,
                                                    xvocab_size, yvocab_size, emb_dim)
                # share parameters
                scope.reuse_variables() 
                # testing model, where output of previous timestep is fed as input 
                ...
                self.decode_outputs_test_bwd, self.decode_states_test_bwd = tf.contrib.legacy_seq2seq.embedding_rnn_seq2seq(
                    self.enc_ip, self.dec_ip_bwd, stacked_gru, xvocab_size, yvocab_size,emb_dim,
                    feed_previous=True)
    ...
    def get_feed(self, X, Y, P, keep_prob):
        feed_dict = {self.enc_ip[t]: X[t] for t in range(self.xseq_len)}
        feed_dict.update({self.labels_fwd[t]: Y[t] for t in range(self.yseq_len)})
        feed_dict.update({self.labels_bwd[t]: P[t] for t in range(self.pseq_len)})
    ...
    def train_batch(self, sess, train_batch_gen):
        # get batches
        batchX, batchY, batchP = train_batch_gen.__next__()
        # build feed
        feed_dict = self.get_feed(batchX, batchY, batchP,keep_prob=0.5)
        _, loss_v = sess.run([self.train_op, self.loss], feed_dict)
        return loss_v

    def eval_step(self, sess, eval_batch_gen):
        # get batches
        batchX, batchY, batchP = eval_batch_gen.__next__()
        # build feed
        feed_dict = self.get_feed(batchX, batchY, batchP, keep_prob=1.)
        loss_v, dec_op_v_fwd, dec_op_v_bwd  = sess.run([self.loss, self.decode_outputs_test_fwd, self.decode_outputs_test_bwd], feed_dict)
        # dec_op_v is a list; also need to transpose 0,1 indices 
        #  (interchange batch_size and timesteps dimensions
        dec_op_v_fwd = np.array(dec_op_v_fwd).transpose([1,0,2])
        dec_op_v_bwd = np.array(dec_op_v_bwd).transpose([1,0,2])
        return loss_v, dec_op_v_fwd, dec_op_v_bwd, batchX, batchY, batchP

    # evaluate 'num_batches' batches
    def eval_batches(self, sess, eval_batch_gen, num_batches):
        losses = []
        for i in range(num_batches):
            loss_v, dec_op_v_fwd, dec_op_v_bwd, batchX, batchY, batchP = self.eval_step(sess, eval_batch_gen)
        ...
        for i in range(self.epochs):
            try:
                ...
                if i and i% (self.epochs/10) == 0: # TODO : make this tunable by the user
                    ...
                    train_loss = self.eval_batches(sess, train_set, 16)
                    # print stats
    ...
    def predict(self, sess, X):
        ...
        dec_op_v_fwd, dec_op_v_bwd = sess.run([self.decode_outputs_test_fwd, self.decode_outputs_test_bwd], feed_dict)
        # dec_op_v is a list; also need to transpose 0,1 indices 
        #  (interchange batch_size and timesteps dimensions
        dec_op_v_fwd = np.array(dec_op_v_fwd).transpose([1,0,2])
        dec_op_v_bwd = np.array(dec_op_v_bwd).transpose([1,0,2])
        ...
        #return dec_op_v
        return np.argmax(dec_op_v_fwd, axis=2), np.argmax(dec_op_v_bwd, axis=2)